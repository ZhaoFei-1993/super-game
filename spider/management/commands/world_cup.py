            ...
            if len(dt_ul) <= 1:
                ...
                for dt_li in dt_ul.find_all('li'):
                    ...
                    if Quiz.objects.filter(host_team=left_team, guest_team=right_team).exists() is not True:
                        ...
                        quiz.host_team_avatar = MEDIA_DOMAIN_HOST + '/images/spider/football/' + left_img_name
                        quiz.guest_team = right_team
                        quiz.guest_team_avatar = MEDIA_DOMAIN_HOST + '/images/spider/football/' + right_img_name
                        quiz.match_name = '世界杯'
                        quiz.begin_at = time
