...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for club in Club.objects.filter(~Q(room_title='HAND俱乐部')):
            ...
            profit_user = 0
            cash_back_sum = 0
            records = Record.objects.filter(roomquiz_id=club.id, is_distribution=True,
            ...
            if ClubProfitAbroad.objects.filter(roomquiz_id=club.id,
                ...
                if len(records) > 0:
                    ...
                    # for cash_back_log in CashBackLog.objects.filter(roomquiz_id=club.id,
                    #                                                 quiz__id__in=quiz_list):
                    ...
                    profit_table.platform_rmb = normalize_fraction(
                        float(platform_sum) * float(coin_price.price), 8)

                    profit_table.profit = normalize_fraction(profit_user, 3)
                    profit_table.profit_rmb = normalize_fraction(float(profit_user) * float(coin_price.price),
                                                                 8)

                    profit_table.profit_total = normalize_fraction(profit_total, 3)
                    profit_table.profit_total_rmb = normalize_fraction(
                        float(profit_total) * float(coin_price.price), 8)
                    ...
                    print('platform_rmb = ',
                          normalize_fraction(float(platform_sum) * float(coin_price.price), 8))
                    ...
                    print('profit_total_rmb',
                          normalize_fraction(float(profit_total) * float(coin_price.price), 8))
