...
class Command(BaseCommand, BaseView):
    help = "Bitcoin & USDT交易数据监控"
    key_transactions = 'key_btc_usdt_transactions'
    ...
    def get_transaction(transaction_hash):
        ...
        """
        eth_wallet = Wallet()
        json_data = eth_wallet.get(url='https://chain.api.btc.com/v3/tx/' + str(transaction_hash))
    ...
    def handle(self, *args, **options):
        btc_usdt_transactions = get_cache(self.key_transactions)
        if btc_usdt_transactions is None or btc_usdt_transactions == 0:
            raise CommandError('未获取到充值')

        confirm_number = 1  # 有效交易确认数

        # 获取所有用户ETH交易hash，只获取交易确认数小于指定值的数据
        user_recharges = UserRecharge.objects.filter(coin_id__in=[Coin.BTC, Coin.USDT], confirmations__lt=confirm_number)
        if len(user_recharges) == 0:
            set_cache(self.key_transactions, 0)
        ...
        for recharge in user_recharges:
            ...
            coin = Coin.objects.get_one(pk=recharge.coin_id)
